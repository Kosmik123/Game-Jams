...
public class Rope : MonoBehaviour
{
    ...
    {
        if (Physics.Linecast(player.position, rope.GetPosition(ropePositions.Count - 2), out var hit, collMask))
        {
    ...
    {
        if (!Physics.Linecast(player.position, rope.GetPosition(ropePositions.Count - 3), out var hit, collMask))
        {
